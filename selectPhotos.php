...
foreach ($archivosSubidos as $k => $v) {
	...
	if(isset($paginas_dobles)){
		...
		for ($i=0; $i < $num_pages; $i++) {
			...
			$im->writeImage($finalFolderImages.'/'.$original_name.'-'.sprintf("%'.04d", $reversePageNumber--).'-1.jpg');
		}
		//SE LIMPIA EL OBJETO
		$im->clear();
		//SEGUNDA PÁGINA
		...
		for ($i=0; $i < $num_pages; $i++) {
			...
			//Se crean y guardan las imagenes para este corte
			$im->writeImage($finalFolderImages.'/'.$original_name.'-'.sprintf("%'.04d", $reversePageNumber--).'-2.jpg');
		}
		$im->clear();
	}else{
		$im = new Imagick();
		...
		for ($i=0; $i < $num_pages; $i++) {
			...
			//Se crean y guardan las imagenes para este corte
			$im->writeImage($finalFolderImages.'/'.$original_name.'-'.sprintf("%'.04d", $reversePageNumber--).'.jpg');
		}
		//SE LIMPIA EL OBJETO
		$im->clear();
	}
